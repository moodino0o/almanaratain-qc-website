...
import { getEmployeeRole } from '../lib/auth';
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const allowed =
      parsed.data.purpose === 'archive'
        ? role === 'administrator'
        : role === 'managerial' || role === 'administrator';
    if (!allowed) {
      res.status(403).json({ error: 'Your role does not allow this upload.' });
      return;
    }
